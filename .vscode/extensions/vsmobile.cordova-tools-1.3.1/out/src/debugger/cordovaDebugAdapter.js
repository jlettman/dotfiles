...
class CordovaDebugAdapter extends vscode_chrome_debug_core_1.ChromeDebugAdapter {
    ...
    launchAndroid(launchArgs, projectType, runArguments) {
        ...
        let cordovaResult = extension_1.cordovaRunCommand(args, workingDirectory).then((output) => {
    ...
    launchIos(launchArgs, projectType, runArguments) {
        ...
        if (launchArgs.target.toLowerCase() === 'device') {
            ...
            return extension_1.cordovaRunCommand(args, workingDirectory).then((output) => {
            ...
            return extension_1.cordovaRunCommand(args, workingDirectory)
                .progress((progress) => {
                this.outputLogger(progress[0], progress[1]);
            }).catch((err) => {
                if (target) {
                    return extension_1.cordovaRunCommand(['emulate', 'ios', '--list'], workingDirectory).then((output) => {
